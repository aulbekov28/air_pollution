...
(function() {
    ...
    function parseDisplayData() {
        //               1      2        3        4          5          6
        var parser = /^\/(\w+)\/(current|(\d{4})\/(\d{1,2})\/(\d{1,2})\/(\d{1,2}))$/;
        var hash = window.location.hash.substr(1) || "";
        var parts = parser.exec(hash) || ["/wind/current", "wind", "current"];
        var type = parts[1], path = parts[2];
        var date = path === "current" ?
            "current" :
    ...
    function navigateTo(type, date) {
        if (date === "current") {
            window.location.hash = ["", type, "current"].join("/");
        } else {
    ...
    function stop() {
        return settingsTask.then(function(settings) {
    ...
    // When the URL hash fragment changes, reload the page using the new values.
    d3.select(window).on("hashchange", function() {
        when(mainTask).then(stop).then(function() {
            bad = false;
    ...
    function init() {
        ...
        function navToHours(offset) {
            var date = displayData.date === "current" ? new Date() : displayData.date;
            date.setHours(date.getHours() + offset);
            navigateTo(displayData.type, date);
        }
     ...
     */
    function plotStations(stations, data, mesh) {
        // Convert station data to GeoJSON format.
        var features = [];
        data.samples.forEach(function(sample) {
            if (isValidSample(sample) && stations[sample.stationId]) {
                features.push({
                    type: "Features",
                    properties: {name: sample.stationId.toString()},
                    geometry: {type: "Point", coordinates: stations[sample.stationId].coordinates}});
            }
        });
    ...
    function formatOverlayValue(v) {
        var recipe = displayData.recipe;
        v = Math.min(v, recipe.max);
        v = Math.max(v, Math.min(recipe.min, 0));
        if (recipe.multiplier) {
            v *= recipe.multiplier;
        }
        return v.toFixed(recipe.precision) + recipe.unit;
    }
     ...
     */
    function interpolateField(stations, data, settings, masks) {
    ...
    // Define a dependency graph of tasks and use promises to let the control flow occur organically. Any errors
    // will cause dependent tasks to be skipped.

    var topoTask         = loadJson(displayData.topography);
    var stationsTask     = loadJson(displayData.stations).then(prepareStations);
    var initTask         = when(true).then(init);
    ...
    function build() {
        var dataTask         = loadJson(displayData.samples).then(prepareData);
        var plotStationsTask = when.all([stationsTask, dataTask, meshTask                ]).then(apply(plotStations));
        var overlayTask      = when.all([stationsTask, dataTask, settingsTask, renderTask]).then(apply(drawOverlay));
        var fieldTask        = when.all([stationsTask, dataTask, settingsTask, renderTask]).then(apply(interpolateField));
        ...
        return when.all([
            ...
            animateTask,
            postInitTask
        ]).then(function() { console.log("main done"); }).otherwise(report);
    }
